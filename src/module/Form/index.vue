<template>
  <el-form v-bind="$attrs" ref="leaseForm" @submit.prevent :model="modelObject" :rules="rules" :inline="inline" :size="size">
    <slot
      v-for="{
        key,
        prop,
    ...
    >
      <el-form-item :key="key ?? prop" :prop="prop" :label="label" :rules="ItemRules" :error="error" :style="style">
        ...
        <template v-if="['radio', 'radioGroup'].includes(type) && radioOption">
          <el-radio-group v-model="modelObject[prop ?? key]" :disabled="radioOption?.disabled" :size="radioOption?.size">
            <el-radio v-for="radioItem in radioOption?.list" :key="radioItem.value" :label="radioItem.label" :disabled="radioItem?.disabled">
              {{ radioItem.value }}
            </el-radio>
          </el-radio-group>
        </template>
        ...
        <template v-if="['checkBox', 'checkBoxGroup'].includes(type) && checkBoxOption">
          <el-checkbox-group v-model="modelObject[prop ?? key]" :disabled="selectOption?.disabled" :size="selectOption?.size">
            <el-checkbox
...
export default defineComponent({
  ...
  props: {
    ...
    modelValue: {
      type: Object as PropType<ModelObject>,
      default: () => {
        return {}
      }
    },
  ...
  setup(props, ctx) {
    ...
    const Header = reactive(header.value as FormOption[])
    const leaseForm = ref<FormInstance>()
    const modelObject = reactive<ModelObject>({})
